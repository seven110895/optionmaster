...
total_data = pd.DataFrame(columns=('Time', 'Underlying', 'Weight_1', 'Weight_2', 'Strike', 'Maturity_1', 'Maturity_2',
                                   ...
                                   'net_pos', 'cash_balance', 'Daily_Return', 'Cost', 'signal', 'Op_delta',
                                   'daily_hedge_pos', 'hedge_future_price', 'hedge_cost', 'cum_hedge_cash',
                                   'realized_hedge_return'))
    ...
    if d in dateList:
        ...
        if signal != "hold":
            ...
            # clean_long means close the original long position and open a new long position
                ...
                buy_list = d1_bid_p + d1_ask_p + d2_bid_p + d2_ask_p
                sell_list = d1_bid + d1_ask + d2_bid + d2_ask
                weight_mtr_list = option_mtr_weight
                cost = option_data.trade_cost(d1_bid, d1_ask, option_mtr_weight[0], 1) \
                       + option_data.trade_cost(d2_bid, d2_ask, option_mtr_weight[1], 1) \
            ...
            temp_return = [dailyTradeTime, perp_underlying] + weight_mtr_list + sell_list + buy_list \
                          + [new_net_pos, d_cash, d_return, cost, signal] + [Op_delta, daily_hedge_pos,
                                                                             hedge_future_price, hedge_cost,
                                                                             cum_hedge_cash, realized_hedge_return]
            ...
            total_data.loc[day_count, 'Daily_Return'] = 0
            total_data.loc[day_count, 'signal':'realized_hedge_return'] = [signal, Op_delta, daily_hedge_pos,
                                                                           hedge_future_price, hedge_cost,
                                                                           cum_hedge_cash, realized_hedge_return]
        day_count += 1
...
total_data.to_csv("D:/optionmaster/Straddle/Straddle_20200228.csv")
